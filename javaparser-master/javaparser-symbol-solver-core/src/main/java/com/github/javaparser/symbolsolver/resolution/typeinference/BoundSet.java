...
public class BoundSet {
    ...
    private boolean thereIsSomeJSuchThatBequalAlphaJ(Set<InferenceVariable> alphas, InferenceVariable beta) {
    ...
    private boolean hasProperty(Set<InferenceVariable> alphas, List<VariableDependency> dependencies) {
        for (InferenceVariable alphaI: alphas) {
            for (InferenceVariable beta: dependencies.stream()
                ...
                if (!hasInstantiationFor(beta) && !thereIsSomeJSuchThatBequalAlphaJ(alphas, beta)) {
                    return false;
                }
            }
        }
